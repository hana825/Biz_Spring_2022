...
public class FileServiceImplV2 implements FileUpService{
	...
	// servlet-context.xml 에 bean으로 설정된 값을 가져와서 upLoadFolder 변수에 담아달라
	...
	public String fileUp(MultipartFile file) throws Exception {
		...
		/*
		 * 파일 업로드를 할 때 원래 파일이름을 그대로 저장하면 해킹 등의 위협에 노출될 수 있다.
		 * 그래서 파일이름 앞에 UUID 문자열을 첨부하여 파일을 저장하고,
		 * 실제 사용자들에게 보여줄 때는 원래 이름을 보여주는 식으로 코드를 작성해야 한다.
		 */
		// UUID 문자열 생성
	...
	public List<FilesVO> filesUp(MultipartHttpServletRequest files) throws Exception {
		...
		/*
		 * fileList를 for()반복문으로 반복하면서 각각의 파일을 fileUp() method에게 보내서
		 * 개별 파일을 업로드 수행. uuID fileName을 return 받기
		 * 
