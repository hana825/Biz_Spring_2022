...
public class NaverBookServiceEx {
	...
	/*
	 * cat와 search에 담긴 값을 전달받아서 Naver에 요청하는 QueryString을 생성하는 method
	 */
	...
	public String getJsonString(String queryString) {
		...
		// queryString(URL, 요청정보)를 사용하여 Network 통해서 보낼 데이터로 생성하기
		try {
...
//			httpCon.setRequestProperty("X-Naver-Client-Id", NaverConfig.NAVER_CLIENT_ID);
			...
			// Naver에게 queryString 정보를 보내고 response해줄 데이터가 있는지 먼저 확인한다.
			...
			if(resCode == 200) {
				...
				// 만약 200코드가 아니면 오류메시지를 받을 통로를 연결하기
