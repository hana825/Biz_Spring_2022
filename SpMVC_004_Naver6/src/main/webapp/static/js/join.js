  ...
  */
  username?.addEventListener("blur", (e) => {
    ...
    fetch(`${rootPath}/user/idcheck/${current.value}`)
      .then((res) => res.text())
      .then((result) => {
        // 서버에서 OK 문자열을 보내면 체크한 username은 사용해도 된다라는
        if (result === "OK") {
  ...
  email?.addEventListener("input", (e) => {
    ...
    if (emailRule.test(current.value)) {
      ok_msg(error, " * 이메일 양식 검사 완료 ☺️");
    } else {
      error_msg(error, " * 이메일 양식을 확인해주세요!");
      // email양식검사를 통과하지 못하면 다음 코드를 실행하지 않도록
      return false;
    } // 양식검사 완료
    fetch(`${rootPath}/user/emailcheck?email=${current.value}`)
      ...
      .then((result) => {
        if (result === "OK") {
          ok_msg(error, " * 사용할 수 있는 이메일입니다☺️");
        } else {
          error_msg(error, " * 이미 등록된 이메일입니다. 다시 입력해주세요!");
        }
      });
  });
