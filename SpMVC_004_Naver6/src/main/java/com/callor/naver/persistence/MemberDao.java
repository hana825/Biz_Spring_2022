...
public interface MemberDao extends GenericDao<MemberVO, String>{
	 ...
	 * 이 method가 잘 작동이 되기 위해서는 Entity설계를 할 때 email 칼럼은 반드시 UNIQUE 설정이 되어 있어야 한다.
	 * 좀 더 정확한 설정을 위하여 NOT NULL 설정도 같이 하는 것이 좋다.
	 */
	public MemberVO findByEmail(String email);
	
	/*
	 * 회원의 ID 찾기 기능을 위하여 전화번호와 email을 사용하여 username을 조회할 수 있는 method 구현
	 * Entity설계를 할 때 UNIQUE(email, tel) 형식으로 제약조건을 설정할 필요가 있다.
