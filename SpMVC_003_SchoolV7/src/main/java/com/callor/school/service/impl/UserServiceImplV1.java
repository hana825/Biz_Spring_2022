...
public class UserServiceImplV1 implements UserService{
	...
	// security-context.xml 에 저장된 bean 불러오기
	...
	public UserVO login(UserVO userVO) {
		...
		if(username.equalsIgnoreCase("hana")
				&& password.equals("12341234")){
			userVO.setName("나");
			userVO.setRole("ADMIN");
			userVO.setEmail("hana825@naver.com");
		} else {
			userVO = null;
		}
		return userVO;
	 ...
	 * 2. 두 번째 이후 회원가입을 실행하는 가입자는 USER role 부여하기
	 */
	@Override
	public UserVO join(UserVO userVO) {
		
		
		// 회원가입한 user의 정보를 저장하기 전에 비밀번호를 암호화하자
		String encPassword = pass.encode(userVO.getPassword());
		userVO.setPassword(encPassword);
		

		// 1. user table에 데이터가 있는지 확인하기 위하여 전체 데이터를 select하기
		List<UserVO> userList = userDao.selectAll();
		// 만약 user table에 데이터가 없으면 null을 return 한다
		if(userList == null || userList.size() < 1) {
			// 데이터가 없으면 최초 가입자이므로 role(권한)을 ADMIN(관리자)로 부여한다
			userVO.setRole("ADMIN");
		} else {
			userVO.setRole("USER");
		}
