...
public class UserControllerV2 {
	 ...
	 * UserService 인터페이스를 상속받은 클래스가 두 개 있다.
	 * User...ImplV1 과 User...ImplV2가 있는데 
	 ...
	 * 매번 페이지 이동때 다시 로그인을 수행한다면 상당히 불편한상황이 될 것이다.
	 * 웹 애플리케이션은 로그인한 사용자정보를 어딘가에 보관을 하고 있다가 다른 페이지로 이동 하였을 때
	 * 그 정보를 참조할 수 있어야 한다.
	 * 웹 어플리케이션은 이러한 유지되어야 할 정보를 서버의 메모리에 보관을 해놓는다.
	 * 
	 * Spring 에서는 이 정보를 HttpSession 클래스를 사용하여 관리한다.
	 * 로그인한 사용자의 정보(VO data)를 HttpSession에 변수로 저장을 해놓는다.
	 * 
	 * session.setAttribute("USER", loginUser);
	 * USER라는 변수에 loginUser 정보를 저장해 놓는다. 이 후에 서버를 재 시작, 종료하기 전까지
	 * USER라는 변수에는 lgoinUser 정보가 계속 담겨 있게 된다.
	 * 임의로 이 정보를 삭제하려면
	...
	public String join() {
		 ...
		 * return null 을 실행하면 return user/join을 실행한 것과 같다.
	...
	public String join(UserVO userVO) {
		 ...
		 * => web browse 주소창에 localhost:8080/url을 입력하고 Enter를 누른 것과 같은 효과
		 */
	 ...
	 * /user/idcheck?username=aaa
	 * 
	 * fetch(`${rootPath}/user/idcheck/${username.value}`)
	 * 만약 username에 aaa 입력했으면
	 * 	/user/idcheck/aaa 처럼 요청 URL 만들어서 요청을 수행하라
	 * 
	 * ID를 email 주소로 사용할 때  PathVariable로 받을 경우﻿ dot(.) 이후의 문자열을 잘라버리는 현상이 있다.
	 * 이때는 정규식(Rexp)을 사용하여 dot(.) 이후 문자열을 포함하여 변수에 저장하도록 변수를 수정한다.
	 * {username:.+} 형식으로 지정한다.
	 * 
	 */
	@ResponseBody
