document.addEventListener("DOMContentLoaded", () => {
  /*
    이 코드의 목적은 nav의 li tag를 클릭했을 때 할 일을 지정하는 것이다.
    화면에 li tag가 여러개가 있다. 여러 개의 li tag에 click event를 설정하려면
    ...
    nav.main_nav에 click event를 설정했지만 e 매개변수의 target 속성을 사용하면 가장 안쪽의 li tag에 접근이 가능하다.
    li tag click이 되면 tagName을 조회해보고 실제로 LI tag가 클릭된 것이면 LI tag의 text를 가져와서 location을 변경하는 용도로 활용한다.
