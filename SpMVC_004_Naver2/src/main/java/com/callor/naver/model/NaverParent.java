 ...
 * JSON으로 응답받은 데이터를 한 번에 VO 리스트로 변환하기 위한 Wrapper 클래스 선언
 * VO를 감싸고 있는 클래스이다. 
 * Parent 클래스가 VO 리스트를 한꺼번에 감싸고 있는 계층구조 데이터 선언
 * 
 * Spring 5.0 이상에서는 사용을 자제해야 한다.(내부 아키텍처가 변해서)
 */
public class NaverParent<VO> {
	 ...
	 * 이때 RestTemplate를 사용하여 데이터를 가져오는데 RestTemplat은 VO를 감싸는 List 변수가 한 개 필요하다.
	 * 그런데 List를 감싸는 VO가 News, Books, Movies들이 각각 다르다.
	 * 이럴때 parent클래스를 각각의 VO를 감싸는 타입으로 복수 정의해야 하는 문제가 있다.
	 * 이때도 Generic의 기능을 사용하여 클래스 선언문에 Generic을 정의해두고, 
