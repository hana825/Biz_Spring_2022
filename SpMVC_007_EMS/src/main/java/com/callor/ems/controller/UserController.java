...
public class UserController {
	 ...
	 * UserVO userVO = new UserVO(); 대신 실행
	 * 
	...
	public String join(@Valid UserVO userVO, BindingResult result) {
		...
		// 인증 메일을 보내고 나면 userVO의 key_ok 변수에는 인증 키가 담겨있는 상태가 된다.
		return "redirect:/user/join_next";
	}
	
	/*
	 * ModelAttribute userVO 코드를 만나면 일단 클라이언트에서 전송되어 userVO에 담긴 데이터가 있는지 확인한다.
	 * 데이터가 있으면 그대로 통과를 하고, 만약 데이터가 없으면 SessionAttribute에 보관중인 userVO 데이터를 가져와서 매개변수 userVO에 세팅한다.
	 */
