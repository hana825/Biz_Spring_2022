...
//		List<AddressVO> addrList = addrService.selectAll();
	...
	public String delete(@RequestParam(name="seq", required= false, defaultValue="0") long seq) {
		addrService.delete(seq);
		return "redirect:/";
	}
	
	/*
	 * Controller의 method 에서 문자열을 return 하면 tiles/layout.xml 파일에서 해당하는 문자열로 선언된 definition을 찾는다.
	 * 해당하는 문자열로 선언된 definition이 있으면 layout.xml에 설정된 대로 tiles가 작동되어 layout을 만들고 rendering하여 응답을 한다.
	 * 만약, layout.xml에 해당하는 definition이 없으면 InternalResourceViewResolver가 작동되어 /views/문자열.jsp 파일을 찾아서 rendering하여 응답한다.
	 */
