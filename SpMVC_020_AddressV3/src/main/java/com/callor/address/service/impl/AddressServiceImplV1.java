...
public class AddressServiceImplV1 implements AddressService {
	...
	// 조건에 맞는 데이터를 가져와서 pagination을 그리기 위한 SearchPage 클래스의 데이터를 만들 것
	@Override
	public void searchAndPage(Model model, SearchPage searchPage) {
		...
		// 1. 전체 데이터 개수가 몇 개인지 알아야 pagination을 구현할 수 있다.
		// 화면 하단의 페이지 번호를 자동으로 계산하기 위함이다.
		// 검색어 조건에 맞는 모든 데이터를 일단 select
		
		// 검색어가 없이 요청이 될 경우 SearchPage 객체의 search 값이 null이 되어
		// 데이터가 검색이 되지 않는다. 때문에 SearchPage 객체의 search 변수값을 ""으로 세팅
		...
		// 선택된 페이지번호에 따라 화면하단 번호 리스트를 유동적으로 보여주기 위한 연산
