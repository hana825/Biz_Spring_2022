...
함수 선언문에 async 키워드를 부착하고 실행 순서를 임의로 조절하기 위하여 
...
const st_num_fetch = (st_num) => {
  ...
  // 이 함수가 true나 false를 return할 것으로 기대하고 작성을 했으나
  // 비동기 방식의 이유로 중간의 return 명령이 실행될 기회를 잃는다.
}; // end st_num_fetch()
...
const save_cb = async () => {
  ...
  // JS 코드를 사용하여 Client Side Validation 을 수행
  ...
  //   // false가 담기면 학번 항목을 clear하고 focus()를 담는다.
  //   // 중복된 학번일 경우가 해당한다.
  //   if (!st_num_yes) {
  //     st_num.value = "";
  //     st_num.focus();
  ...
  // Number() 함수 : JS 에서 문자열형 숫자를 숫자형으로 바꾸는 함수
  // parseInt() 함수 : 문자열형 숫자를 정수형 숫자로 바꾸는 함수

  // 문자열형 숫자를 숫자로 바꾸려고 할 때 숫자형이 아닌 다른 데이터가 포함되어 숫자로 바꿀 수 없을 경우
  // Java에서는 NumberFormatException이 발생하고 코드가 중단된다. 코드 중단을 방지하기 위하여 try-catch로 처리를 해주어야 한다.
  // JS에서는 그러한 상황에 Exception이 발생하는 대신 NaN(Not a Number)이라는 값을 return한다.
  ...
  // 아래의 if() 명령문은 'num_grade에 담긴 값이 NaN이면' 이라고 물어보는 명령이다.
  if (!num_grade) {
  ...
  // 이제 모든 유효성 검사를 정상적으로 통과했으므로 서버로 데이터를 전송!!!!!
